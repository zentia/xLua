...
function xlua.loadType(csType)
    if csType then
        -- 某些类型（如嵌套的委托类型）未注册进 IL2CPP 代码生成表，NativeLoadType 会抛
        -- "no such type"。这里用 pcall 兜底，加载失败返回 nil，避免访问一个类型时
        -- 因为它的某个嵌套类型加载失败而导致整个 CS.X 崩溃。
        local ok, cls = pcall(xlua.NativeLoadType, csType)
