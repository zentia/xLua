...
namespace XLua.Generator
{
    ...
    public class FileExporter
    {
        ...
        /// <summary>
        /// GenLua 按平台落盘根目录：DevAssets/GenLua/{buildTarget 小写}
        /// </summary>
        ...
        {
            return Path.Combine("DevAssets", "CSTypeUsedInLua", buildTarget.ToString().ToLowerInvariant());
        }
        ...
        {
            var platformJson = Path.Combine("DevAssets", "CSTypeUsedInLua", buildTarget.ToString().ToLowerInvariant(), "CSTypeUsedInLua.json");
            if (File.Exists(platformJson))
            {
                return platformJson;
            }

            // 兼容未按平台拆分前的公共配置
            return Path.Combine("DevAssets", "CSTypeUsedInLua.json");
        }
        ...
        {
            // csPath 保留以兼容旧调用签名；产物改为按平台写入 DevAssets/GenLua/{buildTarget 小写}
            ...
            Debug.Log($"[XLua][Gen] Output root: {Path.GetFullPath(genRoot)} (BuildTarget={EditorUserBuildSettings.activeBuildTarget})");
