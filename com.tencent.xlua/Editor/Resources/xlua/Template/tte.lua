...
local TTIf = {
    ...
    isTrue = function () { return self.b }
    isFalse = function () { return not self.b }
    toString = function () {
...
local TTElse = { }
local TTElseif = {
    function constructor(exp) {
        this.b = !!exp;
    }
    function isTrue() { return this.b }
    function isFalse() { return !this.b }
    function toString() {
        error('invalid TTE use. please sure that you use the template literal as tagged template');
    }
}
...
function TaggedTemplateEngine(str, ...) {
    ...
    while (i < #str - 2) {
        i = i + 1
        -- handle str
        if (not #ifStack || ifStack[#ifStack].isTrue()) {
            appendStr(str[i])
        }
        justAddedAnEmptyExp = false

        -- handle exp
        if (exps[i] == IF) error('tte.IF must be used as a function');
...
if (typeof process != 'undefined' && process.env.__TEST == 1) {
    ...
    function assertThrow(a, message) {
        ...
        try {
            ...
            error(`thrown error ${e.message}. does not detected error '${message}'`);
        }
        error(`got result ${res}. does not detected error '${message}'`);
    }
    print('all passed');
}