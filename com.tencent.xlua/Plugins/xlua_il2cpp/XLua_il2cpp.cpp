...
static void* CtorCallback(pesapi_callback_info info)
{
    LuaClassInfoHeader* classInfo = reinterpret_cast<LuaClassInfoHeader*>(pesapi_get_constructor_userdata(info));
...
Il2CppObject* LuaValueToCSRef(Il2CppClass *klass, pesapi_env env, pesapi_value jsval)
{
...
pesapi_value CSRefToLuaValue(pesapi_env env, Il2CppClass *targetClass, Il2CppObject* obj)
{
    ...
    luaVal = TryTranslateBuiltin(env, obj);
...
handle_underlying:
        switch (t)
        {
            ...
            case IL2CPP_TYPE_BOOLEAN:
            {
                ...
                {
                    data.u1 = (uint8_t)pesapi_get_value_bool(env, luaValue);
                    hasValue = true;
                }
            }
            case IL2CPP_TYPE_U1:
            ...
            case IL2CPP_TYPE_I4:
            {
                if (pesapi_is_int32(env, luaValue))
                {
                    data.i4 = (int32_t)pesapi_get_value_int32(env, luaValue);
                    hasValue = true;
                }
                break;
            }
            ...
            case IL2CPP_TYPE_U4:
            {
                if (pesapi_is_uint32(env, luaValue))
                {
                    data.u4 = (uint32_t)pesapi_get_value_uint32(env, luaValue);
                    hasValue = true;
                }
                break;
            }
...
static bool ReflectionWrapper(MethodInfo* method, Il2CppMethodPointer methodPointer, pesapi_callback_info info, bool checkLuaArgument, WrapData* wrapData)
{
    ...
    for (int i = csArgStart; i < method->parameters_count; ++i) 
    {
        ...
        if (isLastArgument && hasParamArray)
        {
            ...
            if (Class::IsValuetype(elementType))
            {
                ...
                {
                    GetValueTypeFromLua(env, pesapi_get_arg(info, j), elementType, addr + valueSize * (j - i + csArgStart));
                }
            }
            else
            {
                for(int j = luaParamStart; j < lua_args_len; ++j)
                {
                    il2cpp_array_setref(array, j - i + csArgStart, LuaValueToCSRef(elementType, env, pesapi_get_arg(info, j)));
                }
            }
            args[i] = array;
            continue;
        ...
        if (Class::IsValuetype(parameterKlass))
        {
            ...
            else if (passedByReference)
            {
                ...
                GetValueTypeFromLua(env, luaValue, underlyClass, storage);
                args[i] = storage;
            }
...
static void ReflectionSetFieldWrapper(pesapi_callback_info info, FieldInfo* field, size_t offset, Il2CppClass* fieldType)
{
    ...
    if (Class::IsValuetype(fieldType))
    {
        if (Class::IsNullable(fieldType))
        {
            ...
            uint32_t valueSize = underlyClass->instance_size - sizeof(Il2CppObject);
            bool hasValue = GetValueTypeFromLua(env, jsValue, underlyClass, storage);
#ifndef UNITY_2021_1_OR_NEWER
            *(static_cast<uint8_t*>(storage) + valueSize) = hasValue;
#else
            *(static_cast<uint8_t*>(storage)) = hasValue;
        ...
        else
        {
            ...
            void* storage = alloca(valueSize);
            bool hasValue = GetValueTypeFromLua(env, jsValue, fieldType, storage);
            if (!hasValue)
            {
                memset(storage, 0, valueSize);
            }
    ...
    {
        void* val = LuaValueToCSRef(fieldType, env, jsValue);
...
puerts::UnityExports* GetUnityExports()
{
    ...
    g_unityExports.LuaValueToCSRef = &LuaValueToCSRef;
...
Il2CppObject* GetModuleExecutor(intptr_t ptr, Il2CppReflectionType* rtype)
{
    ...
    auto type = il2cpp_codegen_class_from_type(rtype->type);
    return LuaValueToCSRef(type, env, func);
}
...
void InitialXLua(pesapi_func_ptr* func_array)
{
    ...
    InternalCalls::Add("PuertsIl2cpp.NativeAPI::SetGlobalType_JSObject(System.Type)", (Il2CppMethodPointer)puerts::SetGlobalType_LuaObject);
