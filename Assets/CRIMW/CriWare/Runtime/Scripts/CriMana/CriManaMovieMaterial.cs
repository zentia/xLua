...
public class CriManaMovieMaterial : CriManaMovieMaterialBase
{
	...
	/**
	 * <summary>Setting for the audio-based adjustment of the starting time when using concatenated playback.</summary>
	 * <remarks>
	 * <para header='Description'>Flag to enable the audio-based adjustment of the starting time when using concatenated playback.</para>
	 *　<para header='Note'>To set a loop from script, use the <see cref='CriMana.Player.SetAudioBaseConcatenation'/> method of the player property.<br/>
	 * This property can be set via the inspector to enable/disable looping at Start. <br/>
	 * If this property is changed after Start, it will not apply to the current playback.</para>
	 * </remarks>
	 */
	public bool audioBaseConcatenation
	{
		get { return _audioBaseConcatenation; }
		set
		{
			if (isMonoBehaviourStartCalled) {
	 ...
	 * <remarks>
	 * <para header='Description'>Advanced audio playback features are available when the Advanced Audio mode is enabled. The default value is true.<br/>
	...
	protected override void SetDataToPlayer()
	{
		...
		player.Loop(loop);
		player.SetAudioBaseConcatenation(audioBaseConcatenation);
	...
	[SerializeField] private bool _audioBaseConcatenation = true;
	[SerializeField] private bool _additiveMode = false;
	[SerializeField] private bool _advancedAudio = true;
...
public abstract class CriManaMovieMaterialBase : CriMonoBehaviour
{
	...
	private bool previousOnApplicationPauseStatus = false;
