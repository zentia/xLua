...
namespace CriTimeline.Mana
{
	...
	public class CriManaMixerBehaviour : PlayableBehaviour {
		...
		static private float cFrameSkipTolerance = float.Epsilon;

		private Guid? m_lastClipId = null;
		private double lastPlayedTime = -1.0;
		...
		private bool PlayMovie(CriManaClipBase clipAsset, int startFrame, double startTime) {
			 ...
			 * otherwise set the new movie to play
			 */
			if (clipAsset != null) {
		...
		private async void ProcessFrameOnSeeking(TimelineClip activeClip, CriManaClipBase clip, double frameTime) {
			...
			if (!enableTimelineScrubPlayback) {
				await Task.Delay(1);
		...
		public override void ProcessFrame(Playable playable, FrameData info, object playerData) {
			...
			bool frameSkipped = m_PlayableDirector.time - m_lastDirectorTime > info.deltaTime + cFrameSkipTolerance;
			bool frameRewinded = m_PlayableDirector.time < m_lastDirectorTime;
			m_lastDirectorTime = m_PlayableDirector.time;
			...
			if (m_CheckPosWithinClip && (frameSkipped || frameRewinded)) {
				if(StopMovie(clipState == ClipState.Play)) { m_movieMixerState = MovieMixerState.Stopping; }
			}
			...
			switch (m_movieMixerState) {
				...
				case MovieMixerState.Playing:
					...
					if (m_boundMovieMaterial.timerType == CriMana.Player.TimerType.User) {
						ulong userTime = (ulong)((frameTime - this.lastPlayedTime) * 1000.0);
			...
			if (m_boundMovieMaterial != null && m_boundMovieMaterial.player != null) {
				...
				} while (IsIntermediateState());    /* Forcing update on intermediate states */
		...
		public override void OnPlayableCreate(Playable playable) {
			...
			if (IsEditMode) {
				...
				bool asmDefEnabled = true;
				try {
					assembly = Assembly.Load("CriMw.Criware.Editor");
				} catch (Exception e) {
				...
				if (assembly != null) {
					try {
						var type = asmDefEnabled ?
									assembly.GetType("CriWare.CriTimeline.Mana.CriManaTimelineEditorSettings") :
									assembly.GetType("CriTimeline.Mana.CriManaTimelineEditorSettings");
						var method = type.GetMethod("IsScrubEnabled", BindingFlags.InvokeMethod | BindingFlags.Static | BindingFlags.Public);
						enableTimelineScrubPlayback = (bool)method.Invoke(null, null);
					} catch (Exception e) {
						Debug.LogError("[CRIWARE] Mana Timeline Preview: failed to load configurations. " + e.Message);
					}
				} else {
					Debug.LogError("[CRIWARE] Mana Timeline Preview: failed to load Unity Editor assembly.");
				}
#endif
