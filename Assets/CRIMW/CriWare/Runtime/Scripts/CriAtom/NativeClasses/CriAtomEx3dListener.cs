...
public class CriAtomEx3dListener : CriDisposable
{
	...
	/**
	 * <summary>Check if the 3D listener can be discarded</summary>
	 * <returns>Status of 3D listener (true = can be discarded, false = cannot be discarded)</returns>
	 * <remarks>
	 * <para header='Description'>Checks if a 3D Listener can be destroyed. <br/>
	 * Calling the <see cref='Dispose'/> function will not destroy a listener if it is attached to an AtomExPlayer still playing audio.<br/>
	 * In this case, you will need to either destroy all AtomExPlayers that have this 3D Listener attached, or attach a different 3D Listener to them in order to destroy the listener.<br/></para>
	 * </remarks>
