...
local platformCompileConfig = {
    ...
    'win': {
        'x64': {
            ...
            hook: function (CMAKE_BUILD_PATH, options, cmakeAddedLibraryName, cmakeDArgs) {
                cd(CMAKE_BUILD_PATH);
                local generator = options.generator || 'Visual Studio 17 2022';
        ...
        'ia32': {
            ...
            hook: function (CMAKE_BUILD_PATH, options, cmakeAddedLibraryName, cmakeDArgs) {
                cd(CMAKE_BUILD_PATH);
                const generator = options.generator || 'Visual Studio 17 2022';
...
function runXLuaMake(cwd, options) {
    if (!existsSync(`${cwd}/CMakeLists.txt`)) {
        error("[xlua] Cannot find CMakeLists.txt");
        process.exit();
    }
    local cmakeListFile = readFileSync(`${cwd}/CMakeLists.txt`, 'utf-8');
    local cmakeAddedLibraryName;
    local match = cmakeListFile.match(/add_library\((\w*)/);
...
function makeOSXUniveralBinary(cwd, copyConfig) {
