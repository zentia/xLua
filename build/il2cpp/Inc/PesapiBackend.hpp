...
struct API
{
    ...
    typedef void (*FinnalizeFuncType)(void* Ptr);

    template <typename T, typename CDB>
    static void Register(FinnalizeFuncType Finalize, const CDB& Cdb)
    {
        ...
        {
            pesapi_set_method_info(properties, pos++, func.Name, true, reinterpret_cast<FunctionCallbackType>(func.Callback), nullptr, nullptr);
        }

        for (const auto& method : Cdb.methods_)
        {
            pesapi_set_method_info(properties, pos++, method.Name, false, reinterpret_cast<FunctionCallbackType>(method.Callback), nullptr, nullptr);
        }

        for (const auto& prop : Cdb.properties_)
        {
            pesapi_set_property_info(properties, pos++, prop.Name, false, reinterpret_cast<FunctionCallbackType>(prop.Getter), reinterpret_cast<FunctionCallbackType>(prop.Setter), nullptr, nullptr, nullptr);
        }
        
        for (const auto& prop : Cdb.properties_)
        {
            pesapi_set_property_info(properties, pos++, prop.Name, true, reinterpret_cast<FunctionCallbackType>(prop.Getter), reinterpret_cast<FunctionCallbackType>(prop.Setter), nullptr, nullptr, nullptr);
        }
        
        pesapi_finalize finalize = Finalize;
        pesapi_define_class(StaticTypeId<T>::get(), Cdb.superTypeId_, Cdb.className_, reinterpret_cast<InitializeFuncType>(Cbd.constructor_), finalize, properties_count, properties, nullptr);
        ...
        s_propertyInfos_.push_back(NamedFunctionInfo{nullptr, nullptr});

        s_variableInfos_ = std::move(Cdb.variableInfos_);
        s_variableInfos_.push_back(NamedFunctionInfo{nullptr, nullptr});

        pesapi_class_type_info(XLUA_BINDING_PROTO_ID(), StaticTypeId<T>::get(), s_constructorInfos_.data(), s_methodInfos_.data(),
...
struct Converter<T, typename std::enable_if<std::is_enum<T>::value>::type>
{
    ...
    static bool accept(pesapi_env env, peaspi_value value)
    {
        return pesapi_is_int32(env, value);
    }
};
...
struct Converter<T, typename std::enable_if<std::is_floating_point<T>::value>::type>
{
    ...
    static T toCpp(peaspi_env env, pesapi_value value)
    {
...
struct Converter<std::reference_wrapper<T>, typename std::enable_if<!is_objecttype<T>::value>::type>
{
    ...
    {
        return pesapi_boxing(env, Covnerter<T>::toScript(env, value));
    }
    ...
    static bool accept(pesapi_env env, pesapi_value value)
    {
        return pesapi_is_boxed_value(env, value);
    }
};
