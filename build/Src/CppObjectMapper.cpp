...
FCppObjectMapper* FCppObjectMapper::Get(lua_State *L)
{
    lua_pushlightuserdata(L, &dummy_idx_tag);
    lua_rawget(L, LUA_REGISTRYINDEX);
    FCppObjectMapper* ret = (FCppObjectMapper*)lua_touserdata(L, -1);
    lua_pop(L, 1);
    return ret;
}

int FCppObjectMapper::LoadCppType(lua_State *L)
{
...
void FCppObjectMapper::Initialize(lua_State *L)
{
    lua_pushlightuserdata(L, &dummy_idx_tag);
...
int FCppObjectMapper::FindOrAddCppObject(lua_State *L, const void* TypeId, void* Ptr, bool PassByPointer)
{
    if (Ptr == nullptr)
    {
        lua_pushnil(L);
...
bool FCppObjectMapper::IsInstanceOfCppObject(lua_State *L, const void* TypeId, int ObjectIndex)
{
...
void FCppObjectMapper::BindCppObject(lua_State *L, LuaClassDefinition* ClassDefinition, void* Ptr, bool PassByPointer)
{
    auto Iter = CDataCache.find(Ptr);
...
void FCppObjectMapper::BindCppObject(lua_State *L, LuaClassDefinition* ClassDefinition, void* Ptr, bool PassByPointer, FObjectCacheNode* CacheNodePtr)
{
    CppObject* obj = (CppObject*)lua_newuserdata(L, sizeof(CppObject));
    obj->Ptr = Ptr;
    obj->TypeId = ClassDefinition->TypeId;
...
void FCppObjectMapper::UnBindCppObject(lua_State *L, LuaClassDefinition* ClassDefinition, void* Ptr)
{
...
void FCppObjectMapper::UnInitialize(lua_State *L)
{
...
int FCppObjectMapper::GetMetaRefOfClass(lua_State *L, const LuaClassDefinition* ClassDefinition)
{
    ...
    if (Iter == TypeIdToMetaMap.end())
    {
        ...
        lua_pushlightuserdata(L, const_cast<JSClassDefinition*>(ClassDefinition));
