...
namespace UnityEditor.UI
{
    ...
    public class GraphicEditor : Editor
    {
        ...
        protected SerializedObject m_AnimationMaterialSerializedObject;
        ...
        protected virtual void OnEnable()
        {
            ...
            RefreshAnimationMaterialSerializedObject();
            SceneView.duringSceneGui += DrawAnchorsOnSceneView;
        }
...
#if OSGAME
        protected Action<bool, Material> m_OnMaterialGUIExt;
#endif
        ...
        protected void AppearanceControlsGUI()
        {
            ...
            RefreshAnimationMaterialSerializedObject();
            if (m_AnimationMaterialSerializedObject != null)
            {
                m_AnimationMaterialSerializedObject.Update();
                EditorGUI.BeginChangeCheck();
                EditorGUILayout.PropertyField(m_Material);
                if (EditorGUI.EndChangeCheck())
                {
                    m_AnimationMaterialSerializedObject.ApplyModifiedProperties();
                    var maskableGraphic = target as MaskableGraphic;
                    if (maskableGraphic != null)
                        maskableGraphic.SetMaterialDirty();
#if OSGAME
                    m_OnMaterialGUIExt?.Invoke(true, m_Material.objectReferenceValue as Material);
#endif
                }
                else
                {
#if OSGAME
                    m_OnMaterialGUIExt?.Invoke(false, null);
#endif
                }
            }
        }
        ...
        private void RefreshAnimationMaterialSerializedObject()
        {
            if (m_AnimationMaterialSerializedObject == null)
            {
                var maskableGraphic = target as MaskableGraphic;
                if (maskableGraphic != null)
                {
                    var animationMaterial = maskableGraphic.animationMaterial;
                    if (animationMaterial != null)
                    {
                        m_AnimationMaterialSerializedObject = new SerializedObject(animationMaterial);
                        m_Material = m_AnimationMaterialSerializedObject.FindProperty("m_Material");
                    }
                }
            }
        }
