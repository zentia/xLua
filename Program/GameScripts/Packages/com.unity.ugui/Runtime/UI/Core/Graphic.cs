...
namespace UnityEngine.UI
{
          ...
          ICanvasElement
    {
        ...
        // osgame ugui opt
        protected static MeshGenerationMethod meshGenerationMethod = MeshGenerationMethod.Default;
        public static bool UseOptMaskMaterial = false;

        protected virtual MeshGenerationMethod CurMeshGenerationMethod() { return meshGenerationMethod; }
        // osgame ugui opt
        ...
        private bool use4DimUvFillMesh = false;

        public bool Use4DimUvFillMesh
        {
            get
            {
                return use4DimUvFillMesh;
            }
            set
            {
                use4DimUvFillMesh = value;
            }
        }


        // Cached and saved values
        [SerializeField] private AnimationMaterial m_AnimationMaterial;
        ...
        ///</code>
        /// </example>
        public virtual Color color { get { return m_Color; } set { if (SetPropertyUtility.SetColor(ref m_Color, value)) MarkVerticesDirty(); } }
        ...
        public virtual bool raycastTarget
        {
            ...
            set
            {
                ...
                {
                    if (m_RaycastTarget)
                        GraphicRegistry.UnregisterRaycastGraphicForCanvas(m_Canvas, this);

                    m_RaycastTarget = value;

                    if (m_RaycastTarget && isActiveAndEnabled)
                        GraphicRegistry.RegisterRaycastGraphicForCanvas(m_Canvas, this);
                }
            }
        }
        ...
        [NonSerialized] protected List<IMaterialModifier> m_CachedMaterialModifier;
		...
		[NonSerialized]
        private Canvas m_registerGraphicCanvas = null;
        ...
        public Int32 GetStatsNameId()
        {
            ...
            if(m_StatsNameId == -1)
            {
                var profileName = $"{canvas?.name?? "<null>"}##{gameObject.name}";
                m_StatsNameId = UGUIStats.RegisterCustomName(profileName);
            }
            return m_StatsNameId;
#else
            return -1;
#endif
        }
        ...
        {
#if UNITY_EDITOR
            if (CanvasUpdateRegistry.EnableRebuildDebugLog)
            {
                Debug.LogFormat(this.gameObject, "[{0}] MarkAllDirty for Graphic {1}", Time.frameCount, this.gameObject.pathToRoot);
            }
#endif
            if (m_SkipLayoutUpdate)
            {
        ...
        public void MarkLayoutDirty()
        {
            ...
            if (CanvasUpdateRegistry.EnableRebuildDebugLog)
            {
                Debug.LogFormat(this.gameObject, "[{0}] MarkLayoutDirty for Graphic {1}", Time.frameCount, this.gameObject.pathToRoot);
            }
#endif
        ...
        {
            if (!rectTransform.manualGraphics)
            {
                MarkVerticesDirty();
            }
        }

        public virtual void MarkVerticesDirty()
        {
            if (!IsActive())
                return;
            ...
            if (CanvasUpdateRegistry.EnableRebuildDebugLog)
            {
                Debug.LogFormat(this.gameObject, "[{0}] MarkVerticesDirty for Graphic {1}", Time.frameCount, this.gameObject.pathToRoot);
            }
#endif
            m_VertsDirty = true;
        ...
        public virtual void MarkMaterialDirty()
        {
            if (!IsActive())
                return;
        ...
        [NonSerialized]
        private int m_LastRebuildLayoutFrame = 0;
        ...
        protected override void OnRectTransformDimensionsChange()
        {
            if (gameObject.activeInHierarchy)
            {
                ...
                {
                    SetVerticesDirty();
                    m_LastRebuildLayoutFrame = Time.frameCount;
                }
                else
                {
                    if (m_LastRebuildLayoutFrame != Time.frameCount)
                    {
                        SetVerticesDirty();
                        SetLayoutDirty();
                    }
        ...
        protected override void OnTransformParentChanged()
        {
			...
			RegisterGraphicForCanvas(m_Canvas);
            SetAllDirty();
        }
        ...
        public AnimationMaterial animationMaterial
        {
            get
            {
                ...
                if (ReferenceEquals(m_AnimationMaterial, null))
                {
                    m_AnimationMaterial = GetComponent<AnimationMaterial>();

                    if (ReferenceEquals(m_AnimationMaterial, null))
                    {
                        m_AnimationMaterial = gameObject.AddComponent<AnimationMaterial>();
                    }
                }
        ...
        {
            Int32 sampleIndex = -1;
            UGUIStats.BeginSample(UGUIStatsSampleId.UGUI,"Graphic.CacheCanvas", ref sampleIndex);
			m_Canvas = null;
#if OSG_PROFILE && !OSG_STATS_DEEP_PROFILING
            m_StatsNameId = -1;
#endif

            m_Canvas = FindCanvas(gameObject);
        ...
        public static Canvas FindCanvas(GameObject go)
        {
            ...
            Canvas canvas = null;
            go.GetComponentsInParent(false, list);
            if (list.Count > 0)
            {
        ...
        public Material materialForAnimationMaterial => m_UIMaterial;
        // Use to substitution m_Material
        protected Material m_UIMaterial
        {
            get
            {
                if (m_AnimationMaterial == null)
                {
                    ...
                    {
                        m_AnimationMaterial = gameObject.AddComponent<AnimationMaterial>();
                        m_AnimationMaterial.material = null;
                    }
                }

                return m_AnimationMaterial.material;
            }
            set
            {
                if (m_AnimationMaterial == null)
                ...
                m_AnimationMaterial.material = value;
            }
        }
        ...
        public virtual Material material
        {
            get
            {
                return (m_UIMaterial != null) ? m_UIMaterial : defaultMaterial;
            }
            set
            {
                if (m_UIMaterial == value)
                    return;
        ...
        public virtual Material materialForRendering
        {
            get
            {
                ...
                {
                    UGUIStats.BeginSample(UGUIStatsSampleId.UGUI,"getMaterialForRenderingOpt", ref sampleIndex);
                    if (m_CachedMaterialModifier != null)
                    {
                        Int32 sampleIndexGetMOdifedMaterial = -1;
                        for(Int32 i = 0; i < m_CachedMaterialModifier.Count; ++i)
                        {
                            var modifier = m_CachedMaterialModifier[i];
                            if ( modifier == null)
                                continue;

                            UGUIStats.BeginSample(UGUIStatsSampleId.UGUI,"GetModifiedMaterial", ref sampleIndexGetMOdifedMaterial);
                            currentMat = modifier.GetModifiedMaterial(currentMat);
                            UGUIStats.EndSampleByIndex(ref sampleIndexGetMOdifedMaterial);
                        }
                    }
                    UGUIStats.EndSampleByIndex(ref sampleIndex);
                }
                else
                {
                    ...
                    UGUIStats.BeginSample(UGUIStatsSampleId.UGUI,"getMaterialForRendering", ref sampleIndex);
        ...
        {
            // Profiler.BeginSample("Graphic.OnEnable");
            base.OnEnable();
            CacheCanvas();
        ...
        protected virtual void UpdateMaterial()
        {
            ...
            Int32 sampleIndex = -1;
            UGUIStats.BeginSample(UGUIStatsSampleId.UGUI,"UpdateMaterial", ref sampleIndex);
            canvasRenderer.SetMaterialAndTexture(materialForRendering, 0, 1, mainTexture);
...
#if OSG_PROFILE
        public static bool DisableAllMeshGenerate = false;
#endif
...
#endif
            Int32 sampleIndex = -1;
            UGUIStats.BeginSample(UGUIStatsSampleId.UGUI,"Graphic.UpdateGeometry", ref sampleIndex);
            switch(CurMeshGenerationMethod())
            {
                case MeshGenerationMethod.NativeArray:
                {
                    Int32 sampleIndexOpt = -1;
                    UGUIStats.BeginSample(UGUIStatsSampleId.UGUI,"Graphic.UpdateGeometryNew.", ref sampleIndexOpt);
                    DoMeshGenerationNativeArray();
                    UGUIStats.EndSampleByIndex(ref sampleIndexOpt);
                    break;
                }
                case MeshGenerationMethod.Default:
                {
                    Int32 sampleIndexOld = -1;
                    UGUIStats.BeginSample(UGUIStatsSampleId.UGUI,"Graphic.UpdateGeometryOld", ref sampleIndexOld);
                    // Debug.LogFast($"[UGUI] UpdateGeometry not opt: {name}({GetType().FullName})");
                    ...
                    UGUIStats.EndSampleByIndex(ref sampleIndexOld);
                    break;
                }
                default:
                {
                    throw new Exception($"UGUI: UpdateGeometry error, Method {CurMeshGenerationMethod()} is not support");
                }
            }
        ...
        protected virtual void DoMeshGenerationNativeArray()
        {
            Int32 sampleIndex = -1;
            UGUIStats.BeginSample(UGUIStatsSampleId.UGUI,"Graphic.UpdateGeometry", ref sampleIndex);
            ...
            {
                if (OnPopulateMeshNativeArray(out var vhNative, 1))
                {
                    vhNative.FillMesh(workerMesh);
            ...
            {
                canvasRenderer.SetMesh(null); // clear the vertex helper so invalid graphics dont draw.
            }
        ...
        private void DoMeshGeneration()
        {
            ...
            s_VertexHelper.FillMesh(workerMesh, Use4DimUvFillMesh);
            canvasRenderer.SetMesh(workerMesh);
        }
...
#pragma warning restore 618
        ...
        protected virtual bool OnPopulateMeshNativeArray( out VertexHelperNative vhNative, Int32 quadCountMultiplier)
        {
            Int32 sampleIndex = -1;
            ...
            vhNative = VertexHelperNative.GetVertexHelper(4 * quadCountMultiplier, AdditionalCanvasShaderChannels.None, use4DimUvFillMesh ? 4 : 2);
            vhNative.Clear();
            if (color == Color.white)
            {
                vhNative.AddVert(new Vector3(v.x, v.y), new Vector2(0f, 0f));
                vhNative.AddVert(new Vector3(v.x, v.w), new Vector2(0f, 1f));
                vhNative.AddVert(new Vector3(v.z, v.w), new Vector2(1f, 1f));
                vhNative.AddVert(new Vector3(v.z, v.y), new Vector2(1f, 0f));
            }
            else
            {
                vhNative.AddVert(new Vector3(v.x, v.y), color, new Vector2(0f, 0f));
                vhNative.AddVert(new Vector3(v.x, v.w), color, new Vector2(0f, 1f));
                vhNative.AddVert(new Vector3(v.z, v.w), color, new Vector2(1f, 1f));
                vhNative.AddVert(new Vector3(v.z, v.y), color, new Vector2(1f, 0f));
            }
        ...
        public void RegisterMaterialModifier(IMaterialModifier modifier)
        {
            if (m_CachedMaterialModifier == null)
            {
                m_CachedMaterialModifier = new List<IMaterialModifier>();
                m_CachedMaterialModifier.Add(null);
            }

            if (ReferenceEquals(modifier, this))
            {
                m_CachedMaterialModifier[0] = modifier;
                return;
            }

            for(int i = 1; i < m_CachedMaterialModifier.Count; i++)
            {
                if (ReferenceEquals(m_CachedMaterialModifier[i], modifier))
                    return;
            }
            m_CachedMaterialModifier.Add(modifier);
        }

        public void UnregisterMaterialModifier(IMaterialModifier modifier)
        {
            if ( m_CachedMaterialModifier == null)
                return;
            ...
            if (ReferenceEquals(modifier, this))
            {
                m_CachedMaterialModifier[0] = null;
                return;
            }
            ...
            for(int i = 1; i < m_CachedMaterialModifier.Count; i++)
            {
                if (ReferenceEquals(m_CachedMaterialModifier[i], modifier))
                {
                    m_CachedMaterialModifier[i] = null;
                }
            }
        }
		...
		protected void RegisterGraphicForCanvas(Canvas canvas)
        {
            //Same, nothing need to to
            if (canvas == m_registerGraphicCanvas)
            {
                return;
            }
            ...
            {
                GraphicRegistry.RegisterGraphicForCanvas(canvas, this);
                m_registerGraphicCanvas = canvas;
            }
        }

        protected void UnregisterGraphicForCanvas()
        {
            if (m_registerGraphicCanvas != null)
            {
                GraphicRegistry.UnregisterGraphicForCanvas(m_registerGraphicCanvas, this);
            }
            m_registerGraphicCanvas = null;
        }

        protected void DisableGraphicForCanvas()
        {
            if (m_registerGraphicCanvas != null)
            {
                GraphicRegistry.DisableGraphicForCanvas(m_registerGraphicCanvas, this);
            }
            m_registerGraphicCanvas = null;
        }
    }
