...
namespace UnityEngine.UI
{
    ...
    public static class StencilMaterial
    {
        private class MatEntry
        {
            ...
            public Mask.SoftMaskParams softMaskRect;
        ...
        {
            return Add(baseMat, stencilID, operation, compareFunction, colorWriteMask, 255, 255,MaterialModifyParams.defaultMaterialModifyParams);
        }
        ...
        /// Add a new material using the specified base and stencil ID.
        /// </summary>
        public static Material Add(Material baseMat, int stencilID, StencilOp operation, CompareFunction compareFunction, ColorWriteMask colorWriteMask, int readMask, int writeMask, MaterialModifyParams materialModifyParams)
        {
            ...
            for (int i = 0; i < listCount; ++i)
            {
                ...
                if (ent.baseMat == baseMat
                    ...
                    && ent.softMaskRect == materialModifyParams.SoftMaskParams)
                {
                    ++ent.count;
            ...
            if (materialModifyParams.EnableSoftMask && newEnt.customMat.HasProperty("_SoftMask"))
            {
                newEnt.softMaskRect = materialModifyParams.SoftMaskParams;
                // newEnt.softMask = materialModifyParams.SoftMask;
                // newEnt.customMat.EnableKeyword("_ENABLE_SOFT_MASK");
                // if (newEnt.softMask != null)
                // {
                //     newEnt.customMat.SetTexture("_SoftMask", newEnt.softMask);
                // }
                newEnt.customMat.SetVector("_SoftMaskRect", newEnt.softMaskRect.SoftMaskRect);
                Vector4 src = newEnt.softMaskRect.SoftMaskSize;
                Vector4 packed = newEnt.softMaskRect.SoftMaskMode == Mask.SoftMaskMode.Horizontal
                    ? new Vector4(src.z, src.w, 1f, 0f)
                    : new Vector4(src.y, src.x, 0f, 1f);
                newEnt.customMat.SetVector("_SoftMaskSize", packed);
                newEnt.customMat.EnableKeyword("_SOFT_MASK_ENABLE");
