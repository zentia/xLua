...
namespace UnityEngine.UI
{
    ...
    public class GraphicRaycaster : BaseRaycaster
    {
        ...
        private static void Raycast(Canvas canvas, Camera eventCamera, Vector2 pointerPosition, IList<Graphic> foundGraphics, List<Graphic> results)
        {
            ...
            // osgame add begin by garraxu
            if (eventCamera != null) // ScreenSpace-overlay mode doesn't have eventCamera, no need to check cullingMask
            {
                bool checkCullingMask = canvas != null &&
                                        ((eventCamera.cullingMask & (1 << canvas.gameObject.layer)) != 0);
                if (!checkCullingMask)
                {
                    // UI相机照不到的UI居然能点击到，这不科学，应该过滤掉
                    return;
                }

                if (!canvas.isRenderVisible)
                {
                    // canvas Render不绘制, 不响应
                    return;
                }
            }
            // osgame add end by garraxu
