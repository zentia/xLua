...
namespace UnityEngine.UI
{
    ...
    public class CanvasScaler : UIBehaviour
    {
        ...
        private float m_AdditionalScaleFactor = 1f;
        private bool m_PendingSoftMaskRefresh;

        /// <summary>
        /// Additional multiplier applied on top of the calculated scale factor.
        /// Works with all scale modes without breaking adaptive behavior.
        /// </summary>
        public float additionalScaleFactor
        {
            get { return m_AdditionalScaleFactor; }
            set
            {
                var additionalScaleFactor = Mathf.Max(0.01f, value);
                if (Mathf.Approximately(m_AdditionalScaleFactor, additionalScaleFactor))
                    return;

                m_AdditionalScaleFactor = additionalScaleFactor;
            }
        }

        private void EnqueueSoftMaskUpdate()
        {
            if (!m_PendingSoftMaskRefresh)
                return;

            if (disableUpdate || !isActiveAndEnabled)
            {
                // 不入队，顺手清掉队列里的残留请求。
                if (m_Canvas != null)
                    Mask.CancelDelayedSoftMaskUpdate(m_Canvas);
                return;
            }
            ...
            Mask.RequestDelayedSoftMaskUpdate(m_Canvas);
            m_PendingSoftMaskRefresh = false;
        }

        private void MarkSoftMaskDirtyAndTryEnqueue()
        {
            m_PendingSoftMaskRefresh = true;
            EnqueueSoftMaskUpdate();
        }

        // sgame_unity2021_predev begin
        // optimize operate, when ui is hiden, update can be disable.
        private bool disableUpdate = false;

        public void DisableCanvasScalerUpdate(bool value)
        {
            disableUpdate = value;
            if (disableUpdate)
            {
                if (Mask.CancelDelayedSoftMaskUpdate(m_Canvas))
                    m_PendingSoftMaskRefresh = true;
                return;
            }

            Handle();
            MarkSoftMaskDirtyAndTryEnqueue();
        }
        ...
        private const float kSafeAreaAspectThreshold = 16f / 9f;

        private Vector4 m_SafeAreaBoundBox = Vector4.zero;

        /// <summary>
        /// Canvas safe area offset in screen pixels: left, top, right, bottom.
        /// It only affects Scale With Screen Size when the screen aspect ratio is below 16:9.
        /// </summary>
        public Vector4 safeAreaBoundBox
        {
            get { return m_SafeAreaBoundBox; }
            set
            {
                m_SafeAreaBoundBox = new Vector4(
        ...
        {
#if OSG_PROFILE && !OSG_GC_PROFILE
            int sampleIndex = -1;
            UGUIStats.BeginSample(UGUIStatsSampleId.UGUI_OnEnable, ProfilerName, ref sampleIndex);
#endif
            base.OnEnable();
            m_Canvas = GetComponent<Canvas>();
            Handle();
...
#if OSG_PROFILE && !OSG_GC_PROFILE
            int sampleIndex = -1;
            UGUIStats.BeginSample(UGUIStatsSampleId.UGUI_OnDisabled, ProfilerName, ref sampleIndex);
#endif
            if (m_Canvas == null)
                m_Canvas = GetComponent<Canvas>();
        ...
        private float GetScaleFactorWithAdditionalScale(float scaleFactor)
        {
            return scaleFactor * m_AdditionalScaleFactor;
        }
        ...
        {
            SetScaleFactor(GetScaleFactorWithAdditionalScale(m_DynamicPixelsPerUnit));
        ...
        {
            SetScaleFactor(GetScaleFactorWithAdditionalScale(m_ScaleFactor));
            SetReferencePixelsPerUnit(m_ReferencePixelsPerUnit);
        }

        private static bool IsScreenAspectBelowSafeAreaThreshold(Vector2 screenSize)
        {
            float shortSide = Mathf.Min(screenSize.x, screenSize.y);
            if (shortSide <= 0f)
                return false;

            float longSide = Mathf.Max(screenSize.x, screenSize.y);
            return longSide / shortSide < kSafeAreaAspectThreshold;
        }

        private Vector2 GetSafeAreaAdjustedScreenSize(Vector2 screenSize)
        {
            if (!IsScreenAspectBelowSafeAreaThreshold(screenSize))
                return screenSize;

            float adjustedWidth = screenSize.x - m_SafeAreaBoundBox.x - m_SafeAreaBoundBox.z;
            float adjustedHeight = screenSize.y - m_SafeAreaBoundBox.y - m_SafeAreaBoundBox.w;
        ...
        protected virtual void HandleScaleWithScreenSize()
        {
            ...
            screenSize = GetSafeAreaAdjustedScreenSize(screenSize);
            ...
            SetScaleFactor(GetScaleFactorWithAdditionalScale(scaleFactor));
            SetReferencePixelsPerUnit(m_ReferencePixelsPerUnit);
        }
        ...
        protected virtual void HandleConstantPhysicalSize()
        {
            ...
            SetScaleFactor(GetScaleFactorWithAdditionalScale(dpi / targetDPI));
