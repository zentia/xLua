namespace UnityEngine.UI
{
    ...
    public static class LayoutUtility
    {
        ...
        {
            var maxW = GetLayoutProperty(rect, e => e.maxWidth, float.NaN);
            if (float.IsNaN(maxW) || maxW == -1 || maxW == 0)
            {
                return Mathf.Max(GetLayoutProperty(rect, e => e.minWidth, 0), GetLayoutProperty(rect, e => e.preferredWidth, 0));
            }
            else
            {
                return Mathf.Min( Mathf.Max(GetLayoutProperty(rect, e => e.minWidth, 0), GetLayoutProperty(rect, e => e.preferredWidth, 0)), maxW);
            }
        }
        ...
        {
            var maxH = GetLayoutProperty(rect, e => e.maxHeight, float.NaN);
            if (float.IsNaN(maxH) || maxH == -1 || maxH == 0)
            {
                return Mathf.Max(GetLayoutProperty(rect, e => e.minHeight, 0), GetLayoutProperty(rect, e => e.preferredHeight, 0));
            }
            else
            {
                return Mathf.Min(Mathf.Max(GetLayoutProperty(rect, e => e.minHeight, 0), GetLayoutProperty(rect, e => e.preferredHeight, 0)), maxH);
            }
        }
