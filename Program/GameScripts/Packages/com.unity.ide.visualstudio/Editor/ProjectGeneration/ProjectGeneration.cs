...
namespace Microsoft.Unity.VisualStudio.Editor
{
	...
	public class ProjectGeneration : IGenerator
	{
		...
		public virtual void Sync()
		{
            ...
            if (args.Contains("-ignoreVisualStudioSync"))
            {
                return;
            }
            // We need the exact VS version/capabilities to tweak project generation (analyzers/langversion)
		...
		{
			return m_AssemblyNameProvider.GetAssemblies(ShouldFileBePartOfSolution).ToList();
		}
		...
		protected virtual string ProjectText(Assembly assembly,
			Dictionary<string, string> allAssetsProjectParts,
			ResponseFileData[] responseFilesData)
		{
		...
		protected virtual void AppendReference(string fullReference, StringBuilder projectBuilder)
		{
		...
		public virtual string ProjectFile(Assembly assembly)
		{
			return Path.Combine(ProjectDirectory, $"{m_AssemblyNameProvider.GetAssemblyName(assembly.outputPath, assembly.name)}.csproj");
		}

#if UNITY_EDITOR_WIN
		protected static readonly Regex InvalidCharactersRegexPattern = new Regex(@"\?|&|\*|""|<|>|\||#|%|\^|;", RegexOptions.Compiled);
#else
		protected static readonly Regex InvalidCharactersRegexPattern = new Regex(@"\?|&|\*|""|<|>|\||#|%|\^|;|:", RegexOptions.Compiled);
#endif
		...
		protected virtual string GetLangVersion(Assembly assembly)
		{
		...
		protected virtual void SetAnalyzerAndSourceGeneratorProperties(Assembly assembly, ResponseFileData[] responseFilesData, ProjectProperties properties)
		{
		...
		protected virtual void GetProjectFooter(StringBuilder footerBuilder)
		{
		}
		...
		private IEnumerable<SolutionProjectEntry> ToProjectEntries(IEnumerable<Assembly> assemblies)
		{
			foreach (var assembly in assemblies)
				yield return new SolutionProjectEntry()
				{
					...
					Name = Path.GetFileNameWithoutExtension(ProjectFile(assembly)),
