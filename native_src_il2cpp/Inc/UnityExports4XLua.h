...
typedef bool (*WrapFuncPtr)(MethodType* method, MethodPointer methodPointer, const lua_State* luaState, bool checkArgument, struct WrapData* wrapData);
typedef v8::FunctionCallback FunctionCallbackFunc;

typedef void (*FieldWrapFuncPtr)(const lua_State* luaState, void* fieldInfo, size_t offset, void* typeInfo);

typedef void TypeIdType;

typedef void (*SetNativePtrFunc)(lua_State* obj, void* ptr, void* type_id);

typedef void* (*CreateLuaArrayBufferFunc)(lua_State* env, void* buffer, size_t length);
...
typedef void* (*LuaValueToCSRefFunc)(const void *typeId, lua_State* luaState);

typedef const void* (*CSharpTypeToTypeIdFunc)(const void *type);

typedef void* (*CStringToCSharpStringFunc)(const char* str);

typedef pesapi_value (*TryTranslateFunc)(lua_State* luaState, const void* obj);
...
typedef void* (*GetPersistentObjectFunc)(lua_State* env, const PersistentObjectInfo* objectInfo);

typedef void* (*GetJSObjectValueFunc)(const PersistentObjectInfo* objectInfo, const char* key, const void* type);

typedef void* (*GetModuleExecutorFunc)(lua_State* env);
...
struct UnityExports
{
    ...
    CreateLuaArrayBufferFunc CreateJSArrayBuffer = nullptr;
